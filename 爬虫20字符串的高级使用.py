...
字符串的常见操作:
    ...
    查找内容 index() index()方法的功能和find()方法一致,唯一的区别是要查找的子串,如果在字符串中没有出现,find()方法返回-1,但是index()方法直接报错
    ...
    rfind()和rindex()方法适用于查找子串在字符串中出现了多次的情况
    右查找内容   rfind 代表字符串从右开始查找子串
    右查找子串   rindex 代表字符串从右边开始查找子串,但是和index方法一样,如果存在返回索引值,反之如果子串不在字符串中直接报错
'''

...
#个人信息页面是utf-8   但是还报了编码错误 因为并没有进入到个人信息页面而是跳转到了登陆页面
#在处理涉及到个人账户登陆的网页上如果存在数据的编码异常则使用gb2312编码不能使用utf-8编码格式
...
#对浏览器发出的响应进行编码,获取响应的数据
content=response.read().decode('gb2312')
print(content)
#在headers中cookie或者session决定了网站的反爬验证
#cookie中包含着用户的登陆信息  如果有登陆之后的cookie那么我们可以将携带cookie进入任何界面
#refer  判断当前路径是不是从上一个路径进入的  如果是上一个路径进入的则正常 反之报错
#一般refer当作图片的防盗链